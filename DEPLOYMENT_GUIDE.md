...
#### 1. ヘルスチェック失敗（"サービスが利用できません"エラー）
**症状**: デプロイ後にヘルスチェックが連続で失敗する

**原因**:
- ポート設定の不一致（Railway: $PORT vs VOICEVOX: 50021）
- 起動時間の不足（初回は5-10分かかる）
- メモリ不足

**解決方法**:
1. **railway.toml** で初期遅延を増加:
```toml
[deploy]
healthcheckInitialDelay = 300  # 5分に延長
healthcheckTimeout = 300      # タイムアウトも延長
```
...
1. **再起動ポリシーを調整**:
```toml
restartPolicyMaxRetries = 1  # 再試行回数を削減
```
